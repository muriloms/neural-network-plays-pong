    ...
    public List<double> CalcOutput(List<double> inputValues, List<double> desiredOutput)
    {
        ...
        for (int i = 0; i < numHidden + 1; i++)
        {
            ...
            for (int j = 0; j < layers[i].numNeurons; j++)
            {
                ...
                if (i == numHidden)
                {
                    layers[i].neurons[j].outputs = ActivationFunctionO(N);
                }
                else
                {
                    layers[i].neurons[j].outputs = ActivationFunction(N);
                }
                
                outputValues.Add(layers[i].neurons[j].outputs);
                currentInput = 0;
            }
        }
    ...
    public void UpdateWeights(List<double> outputs, List<double> desiredOutput)
    {
        ...
        for(int i = numHidden; i >= 0; i--)
        {
            for(int j = 0; j < layers[i].numNeurons; j++)
            {
                ...
                {
                    layers[i].neurons[j].errorGradient = layers[i].neurons[j].outputs * (1-layers[i].neurons[j].outputs);
